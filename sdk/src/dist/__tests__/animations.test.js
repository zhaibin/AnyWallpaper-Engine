...
describe('Animations module', () => {
    ...
    beforeEach(() => {
        ...
        delete window.__anyWP_animationsPaused;
    ...
    describe('pause', () => {
        test('should pause all videos', () => {
            ...
            expect(mockVideo.__anyWP_wasPlaying).toBe(true);
        });
        ...
        test('should intercept requestAnimationFrame', () => {
            ...
            expect(window.__anyWP_originalRAF).toBe(originalRAF);
        ...
        test('should set animation paused flag', () => {
            Animations.pause();
            expect(window.__anyWP_animationsPaused).toBe(true);
        });
    ...
    describe('resume', () => {
        ...
        test('should restore requestAnimationFrame', () => {
            ...
            expect(window.__anyWP_originalRAF).toBeUndefined();
        });
        ...
        test('should resume previously playing audio', async () => {
            ...
            expect(mockAudio.__anyWP_wasPlaying).toBeUndefined();
        });
        test('should clear animation paused flag', () => {
            Animations.pause();
            expect(window.__anyWP_animationsPaused).toBe(true);
            Animations.resume();
            expect(window.__anyWP_animationsPaused).toBe(false);
        });
        test('should not resume if not paused', () => {
            const consoleLogSpy = jest.spyOn(console, 'log').mockImplementation(() => { });
            Animations.resume();
            expect(consoleLogSpy).not.toHaveBeenCalledWith(expect.stringContaining('Auto-resuming'));
            consoleLogSpy.mockRestore();
        });
        test('should handle play errors gracefully', async () => {
            mockVideo.play = jest.fn(() => Promise.reject(new Error('Play failed')));
            Object.defineProperty(mockVideo, 'paused', { value: false, writable: true });
            Animations.pause();
            Animations.resume();
