// AnyWP Engine SDK v2.1.9 - JavaScript Bridge
...
var AnyWPBundle = (function (exports) {
    ...
    function setupWebMessageListener() {
        const platform = detectPlatform();
        if (platform === 'unknown') {
            log$1.info('Native bridge not available (platform: unknown)');
            return;
        }
        ...
        log$1.info(`Setting up WebMessage listener (EARLY) for platform: ${platform}`);
        ...
        try {
            ...
            log$1.error('Failed to setup WebMessage listener:', error);
        }
    }
    /**
    ...
    function sendToFlutter(type, data = {}) {
        const platform = detectPlatform();
        if (platform === 'unknown') {
            log$1.warn('Native bridge not available, cannot send message to Flutter');
            return false;
        }
        const message = {
            type: type,
        ...
        log$1.info(`[SendToFlutter] Sending message (${platform}):`, type);
