...
describe('Core - Initialization', () => {
    ...
    test('should log screen dimensions', () => {
        initializeAnyWP(mockAnyWP);
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('[AnyWP]'), expect.stringContaining('[INFO]'), expect.stringContaining('Screen: 1920x1080'));
    });
    test('should log DPI scale', () => {
        initializeAnyWP(mockAnyWP);
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('[AnyWP]'), expect.stringContaining('[INFO]'), expect.stringContaining('DPI Scale: 1x'));
    });
    ...
    test('should log debug mode enabled message', () => {
        initializeAnyWP(mockAnyWP);
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('[AnyWP]'), expect.stringContaining('[INFO]'), expect.stringContaining('Debug mode ENABLED automatically'));
    });
});
...
describe('Core - Utility methods', () => {
    ...
    test('openURL should log message', () => {
        const consoleSpy = jest.spyOn(console, 'log').mockImplementation(() => { });
        AnyWP.openURL('https://example.com');
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Opening URL: https://example.com'));
        consoleSpy.mockRestore();
    });
    ...
    test('ready should log message', () => {
        const consoleSpy = jest.spyOn(console, 'log').mockImplementation(() => { });
        AnyWP.ready('TestWallpaper');
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Wallpaper ready: TestWallpaper'));
        consoleSpy.mockRestore();
    });
    test('openURL should fallback to window.open if WebView not available', () => {
        ...
        AnyWP.openURL('https://example.com');
        expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Native bridge not available'));
