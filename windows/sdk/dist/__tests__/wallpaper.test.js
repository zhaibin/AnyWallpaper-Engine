...
describe('Wallpaper module', () => {
    ...
    beforeEach(() => {
        ...
        // Mock window.AnyWP
        window.AnyWP = mockSDK;
    });
    describe('initWallpaperController', () => {
        it('should initialize wallpaper controller', () => {
            initWallpaperController(mockSDK);
            expect(mockSDK._log).toHaveBeenCalledWith('[Wallpaper] Wallpaper controller initialized successfully', true);
        });
        it('should expose Wallpaper API', () => {
            initWallpaperController(mockSDK);
            const wallpaperAPI = window.AnyWP.Wallpaper;
        ...
        it('should return correct initial state', () => {
            initWallpaperController(mockSDK);
            const state = window.AnyWP.Wallpaper.getState();
        ...
        it('should handle initialization errors', () => {
            ...
            initWallpaperController(mockSDK);
            expect(mockSDK._log).toHaveBeenCalledWith(expect.stringContaining('[Wallpaper] ERROR initializing controller'), true);
            // Restore
    ...
    describe('setInteractive', () => {
        ...
        it('should handle missing chrome.webview', () => {
            ...
            const wallpaperAPI = window.AnyWP.Wallpaper;
            wallpaperAPI.setInteractive(true);
            expect(mockSDK._log).toHaveBeenCalledWith(expect.stringContaining('[Wallpaper] ERROR: chrome.webview not available'), true);
        });
    });
    describe('forceMouseOverEvent', () => {
        ...
        it('should handle null element', () => {
            ...
            const wallpaperAPI = window.AnyWP.Wallpaper;
            wallpaperAPI.forceMouseOverEvent(100, 200);
            expect(mockSDK._log).toHaveBeenCalledWith(expect.stringContaining('[Wallpaper] ERROR: No element at position'), true);
        });
        it('should dispatch custom wallpaper event', () => {
            initWallpaperController(mockSDK);
            const mockDocDispatchEvent = jest.fn(() => true);
            document.dispatchEvent = mockDocDispatchEvent;
            document.elementFromPoint = jest.fn(() => mockElement);
            const wallpaperAPI = window.AnyWP.Wallpaper;
            wallpaperAPI.forceMouseOverEvent(100, 200);
    ...
    describe('error handling', () => {
        it('should handle errors in forceMouseOverEvent', () => {
            ...
            const wallpaperAPI = window.AnyWP.Wallpaper;
            wallpaperAPI.forceMouseOverEvent(100, 200);
            expect(mockSDK._log).toHaveBeenCalledWith(expect.stringContaining('[Wallpaper] EXCEPTION in forceMouseOverEvent'), true);
        });
        it('should handle errors in setInteractive', () => {
            initWallpaperController(mockSDK);
            // Force error by making postMessage throw
            window.chrome.webview.postMessage = jest.fn(() => {
                throw new Error('Test error');
            });
            const wallpaperAPI = window.AnyWP.Wallpaper;
            wallpaperAPI.setInteractive(true);
            expect(mockSDK._log).toHaveBeenCalledWith(expect.stringContaining('[Wallpaper] EXCEPTION in setInteractive'), true);
        });
...
//# sourceMappingURL=wallpaper.test.js.map