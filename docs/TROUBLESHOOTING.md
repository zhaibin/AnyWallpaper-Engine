...
### 2. LNK1104: cannot open file 'anywallpaper_engine_plugin.lib'

**Error:**
```
LINK : fatal error LNK1104: cannot open file '..\plugins\anywallpaper_engine\Debug\anywallpaper_engine_plugin.lib'
```
...
```cpp
extern "C" {
__declspec(dllexport) void AnyWallpaperEnginePluginRegisterWithRegistrar(
...
Check console logs:
```
[AnyWallpaper] Finding WorkerW for Windows 11...
[AnyWallpaper] WorkerW found (Win11): 0x...
```
...
```
[AnyWallpaper] ERROR: WorkerW not found via Win11 method
```
...
```
[AnyWallpaper] WebView2 environment created
[AnyWallpaper] WebView2 controller created
[AnyWallpaper] Navigating to: https://...
```
...
// Then try actual URL
await AnyWallpaperEngine.navigateToUrl('https://www.bing.com');
```
...
**Diagnosis:**
```
[AnyWallpaper] Mouse transparency enabled
```
...
if (!webview_controller_) {
  std::cout << "[AnyWallpaper] ERROR: Controller is null" << std::endl;
  return false;
}
```
...
**In C++:**
```cpp
#define AnyWallpaper_DEBUG 1

#ifdef AnyWallpaper_DEBUG
  #define AnyWallpaper_LOG(msg) std::cout << "[AnyWallpaper] " << msg << std::endl
#else
  #define AnyWallpaper_LOG(msg)
#endif
```
...
Start-Process "build\windows\x64\runner\Debug\anywallpaper_engine_example.exe" -RedirectStandardOutput stdout.log -RedirectStandardError stderr.log
...
**Workaround:** Disable other wallpaper software before using AnyWallpaper Engine
...
await AnyWallpaperEngine.navigateToUrl(url);
```
