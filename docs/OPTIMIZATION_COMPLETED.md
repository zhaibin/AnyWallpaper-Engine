...
**日志示例**:
```
[AnyWP] [ResourceTracker] Tracking window: 0x... (Total: 1)
[AnyWP] [ResourceTracker] Untracked window: 0x... (Remaining: 0)
```
...
**日志示例**:
```
[AnyWallpaper] [Retry] Attempt 1 of 3
[AnyWallpaper] [Retry] Initialization failed, retrying in 1 second...
[AnyWallpaper] [Retry] Attempt 2 of 3
```
...
**日志示例**:
```
[AnyWallpaper] [Security] Added to blacklist: file:///c:/windows
[AnyWallpaper] [Security] Navigation blocked: file:///c:/windows/system32
[AnyWallpaper] [Security] Navigation allowed: https://www.bing.com
```
...
**日志示例**:
```
[AnyWallpaper] [Performance] Reusing existing WebView2 environment
```
...
**日志示例**:
```
[AnyWallpaper] [Maintenance] Performing periodic cleanup...
[AnyWallpaper] [Cache] Clearing browser cache via reload...
[AnyWallpaper] [Cache] Page reloaded
```
...
**日志示例**:
```
[AnyWallpaper] [Security] Configuring permissions...
[AnyWallpaper] [Security] Denied permission: 2 (MICROPHONE)
[AnyWallpaper] [Security] Permissions configured
[AnyWallpaper] [Security] Setting up security handlers...
[AnyWallpaper] [Security] Security handlers installed
```
...
```dart
// 所有优化自动启用
await AnyWallpaperEngine.initializeWallpaper(
...
```dart
// 首次启动慢（创建环境）
await AnyWallpaperEngine.initializeWallpaper(url: url1);

await AnyWallpaperEngine.stopWallpaper();

// 再次启动快（复用环境）⚡ 快 75%
await AnyWallpaperEngine.initializeWallpaper(url: url2);
```
...
### 自动生成的日志文件
- `AnyWallpaper_errors.log` - 错误日志（自动创建）
  - 时间戳 + 错误详情
  - 用于故障诊断
...
### Debug 版本 ✅
- 路径: `build\windows\x64\runner\Debug\anywallpaper_engine_example.exe`
- 特性: 完整日志输出，便于调试
- 用途: 开发测试

### Release 版本 ✅
- 路径: `build\windows\x64\runner\Release\anywallpaper_engine_example.exe`
- 特性: 性能优化，体积精简
- 用途: 生产环境
...
### 启用严格安全模式
```cpp
// 在 AnyWallpaperEnginePlugin 构造函数中
url_validator_.ClearWhitelist();
