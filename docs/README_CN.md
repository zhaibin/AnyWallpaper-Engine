...
AnyWP 是一个 Flutter Windows 插件，可以将 WebView2 嵌入到 Windows 桌面的 WorkerW 层中，作为动态壁纸显示在桌面图标下方。
...
<script>
  ...
  AnyWP.resetPosition('#element');  // 复位
  
  // 点击
  AnyWP.onClick('#button', (x, y) => console.log('点击', x, y));
</script>
```
...
AnyWallpaper Engine/
├── lib/                          # Dart API
│   └── anywallpaper_engine.dart
├── windows/                      # Windows 平台实现
│   ├── anywallpaper_engine_plugin.h     # 插件头文件
│   ├── anywallpaper_engine_plugin.cpp   # 核心实现（WorkerW 逻辑）
│   ├── CMakeLists.txt            # 构建配置
...
// 导航到新 URL
await AnyWallpaperEngine.navigateToUrl('https://newurl.com');
...
\`\`\`
[AnyWallpaper] Plugin initialized
[AnyWallpaper] ========== Initializing Wallpaper ==========
[AnyWallpaper] URL: https://www.bing.com
[AnyWallpaper] Mouse Transparent: true
[AnyWallpaper] OS Detected: Windows 11
[AnyWallpaper] Finding WorkerW for Windows 11...
[AnyWallpaper] WorkerW found (Win11): 0x...
[AnyWallpaper] Creating WebView host window...
[AnyWallpaper] WebView host window created: 0x...
[AnyWallpaper] WebView host parented to WorkerW
[AnyWallpaper] Mouse transparency enabled
[AnyWallpaper] Setting up WebView2...
[AnyWallpaper] WebView2 environment created
[AnyWallpaper] WebView2 controller created
[AnyWallpaper] Navigating to: https://www.bing.com
[AnyWallpaper] ========== Initialization Complete ==========
\`\`\`
