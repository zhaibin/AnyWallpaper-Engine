...
AnyWallpaper Engine/
├── lib/
│   └── anywallpaper_engine.dart           # Dart API
├── windows/
│   ├── anywallpaper_engine_plugin.h       # 插件头文件（含优化类）
│   ├── anywallpaper_engine_plugin.cpp     # 核心实现（含所有优化）
│   ├── CMakeLists.txt
...
// 导航到新 URL
await AnyWallpaperEngine.navigateToUrl('https://new-url.com');
```
...
- ✅ flutter_windows.dll
- ✅ anywallpaper_engine_plugin.dll
- ✅ data/ 目录
...
**完整源码**:
```bash
git clone https://github.com/YOUR_USERNAME/anywallpaper_engine.git
cd anywallpaper_engine
.\setup_webview2.bat
.\build_and_run.bat
```
