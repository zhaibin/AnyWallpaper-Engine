...
import type { AnyWPSDK, DraggableOptions, DraggableData, Position } from '../types';
...
export const DragHandler = {
  ...
  makeDraggable(anyWP: AnyWPSDK, element: string | HTMLElement, options: DraggableOptions = {}) {
    ...
    if (persistKey) {
      Storage.load(anyWP, persistKey, function(savedPos: any) {
        if (savedPos && el) {
    ...
    el.draggable = false;
    el.ondragstart = function(e: DragEvent) { 
      e.preventDefault(); 
      return false; 
    };
    
    el.onselectstart = function(e: Event) {
      e.preventDefault();
      return false;
    };
    
    // Register global mouse handler
    function handleGlobalMouse(event: Event) {
  ...
  removeDraggable(anyWP: AnyWPSDK, element: string | HTMLElement) {
    let el: HTMLElement | null;
    if (typeof element === 'string') {
      el = document.querySelector(element) as HTMLElement | null;
    } else {
  ...
  resetPosition(anyWP: AnyWPSDK, element: string | HTMLElement, position?: { left: number; top: number }): boolean {
    let el: HTMLElement | null;
    if (typeof element === 'string') {
      el = document.querySelector(element) as HTMLElement | null;
    } else {
