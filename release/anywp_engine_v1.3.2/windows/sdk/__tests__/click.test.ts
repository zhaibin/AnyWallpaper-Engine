...
import type { AnyWPSDK, ClickCallback } from '../types';
...
describe('Click handler module', () => {
  ...
  describe('handleClick', () => {
    test('should trigger callback when click is in bounds', () => {
      const callback: ClickCallback = jest.fn();
    ...
    test('should only trigger first matching handler', () => {
      const callback1: ClickCallback = jest.fn();
      const callback2: ClickCallback = jest.fn();
      const bounds = Bounds.calculate(mockElement, 1);
  ...
  describe('onClick', () => {
    ...
    test('should update existing handler for same element', () => {
      const callback1: ClickCallback = jest.fn();
      const callback2: ClickCallback = jest.fn();
      ...
      expect(mockAnyWP._clickHandlers.length).toBe(1);
      expect(mockAnyWP._clickHandlers[0]?.callback).toBe(callback2);
    });
