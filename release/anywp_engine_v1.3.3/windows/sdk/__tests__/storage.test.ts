...
describe('Storage module', () => {
  ...
  describe('save', () => {
    ...
    test('should handle save errors gracefully', () => {
      ...
      expect(() => {
        Storage.save(mockAnyWP, 'key', { nested: 'value' });
      }).not.toThrow();
  ...
  describe('load', () => {
    ...
    test('should timeout if no response from native', async () => {
      ...
      try {
        ...
        const loadedValue = await resultPromise;
        expect(loadedValue).toBeNull();
      } finally {
        jest.useRealTimers();
      }
  ...
  describe('saveElementPosition', () => {
    test('should save element position', () => {
      ...
      expect(mockAnyWP._persistedState[key]).toEqual({ left: x, top: y });
      expect(mockWebview.postMessage).toHaveBeenCalledWith({
        type: 'saveState',
        key: key,
        value: JSON.stringify({ left: x, top: y })
      });
    });
    ...
    test('should fallback to localStorage for element position', () => {
      ...
      expect(stored).toBe(JSON.stringify({ left: x, top: y }));
    });
  });
});
