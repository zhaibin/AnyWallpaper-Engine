...
const removeRegisteredHandlers = () => {
  const handlers = (Events as any)._eventHandlers || {};
...
describe('Events module', () => {
  ...
  let mockAnimationsHandler: any;
  
  beforeEach(() => {
    removeRegisteredHandlers();
    (Events as any)._setupCompleted = false;
    (Events as any)._eventHandlers = {};
  ...
  describe('setup', () => {
    ...
    test('should setup interaction mode listener', () => {
      Events.setup(mockAnyWP, mockClickHandler, mockAnimationsHandler);
      
      const event = new CustomEvent('AnyWP:interactionMode', {
        detail: { enabled: false }
      });
      window.dispatchEvent(event);
      
      expect(mockAnyWP.interactionEnabled).toBe(false);
    });
  ...
  describe('onMouse', () => {
    test('should register mouse callback', () => {
      const callback: MouseCallback = jest.fn();
    ...
    test('should register multiple mouse callbacks', () => {
      const callback1: MouseCallback = jest.fn();
      const callback2: MouseCallback = jest.fn();
  ...
  describe('onKeyboard', () => {
    test('should register keyboard callback', () => {
      const callback: KeyboardCallback = jest.fn();
