...
**推荐：预编译包**（最简单集成）
```
anywp_engine_v2.1.4_precompiled.zip
```

**可选：源码包**（需要自定义修改）
```
anywp_engine_v2.1.4_source.zip
```
...
如果你使用了带版本号的文件夹名（如 `anywp_engine_v2.1.4_precompiled`），更新步骤：
...
2. **解压新版本**：
   ```bash
   # 解压 anywp_engine_v2.1.4_precompiled.zip 到 packages\
   ```
...
3. **更新 pubspec.yaml**：
   ...
   dependencies:
     anywp_engine:
       path: ./packages/anywp_engine_v2.1.4_precompiled  # 更新版本号
   ```
...
```bash
# Windows
copy packages\anywp_engine_v2.1.4\lib\dart\anywp_engine.dart packages\anywp_engine_v2.1.4\lib\
```
...
```bash
cd packages\anywp_engine_v2.1.4
verify.bat
```
...
- [ ] 下载预编译包 (`anywp_engine_v2.1.4_precompiled.zip`)
