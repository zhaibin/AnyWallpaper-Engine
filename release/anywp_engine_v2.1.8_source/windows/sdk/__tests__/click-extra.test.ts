...
describe('Click handler - Parameter validation', () => {
  ...
  beforeEach(() => {
    ...
    (window as any).AnyWP = mockAnyWP;
  });
  
  afterEach(() => {
    delete (window as any).AnyWP;
  });
  ...
  it('should throw error when element is a function (wrong parameter order)', () => {
    ...
    expect(() => {
      ClickHandler.onClick(mockAnyWP, wrongCallback as any, jest.fn());
  ...
  it('should throw error when callback is not a function', () => {
    expect(() => {
      ClickHandler.onClick(mockAnyWP, '#test-element', 'not-a-function' as any);
    }).toThrow('[AnyWP] onClick: callback must be a function');
  });
  
  it('should throw error when callback is null', () => {
    expect(() => {
      ClickHandler.onClick(mockAnyWP, '#test-element', null as any);
    }).toThrow('[AnyWP] onClick: callback must be a function');
  });
  
  it('should throw error when callback is undefined', () => {
    expect(() => {
      ClickHandler.onClick(mockAnyWP, '#test-element', undefined as any);
    }).toThrow('[AnyWP] onClick: callback must be a function');
  });
});
...
describe('Click handler - Duplicate registration', () => {
  ...
  it('should log warning when registering duplicate handler', () => {
    ...
    expect(consoleWarnSpy).toHaveBeenCalledWith(
      expect.stringContaining('[AnyWP] Element already has click handler'),
      expect.stringContaining('test-element')
    );
    expect(consoleWarnSpy).toHaveBeenCalledWith(
      expect.stringContaining('[AnyWP] Existing handler callback:'),
      expect.any(String)
    );
    expect(consoleWarnSpy).toHaveBeenCalledWith(
      expect.stringContaining('[AnyWP] New callback (ignored):'),
      expect.any(String)
    );
  ...
  it('should not register when element not found', () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      expect.stringContaining('[AnyWP] Element not found:'),
      '#nonexistent'
    );
...
describe('Click handler - Auto-refresh mechanisms', () => {
  ...
  beforeEach(() => {
    // Mock ResizeObserver
    (global as any).ResizeObserver = jest.fn().mockImplementation((_callback) => ({
      observe: jest.fn(),
      unobserve: jest.fn(),
      disconnect: jest.fn()
    }));
    ...
    (global as any).IntersectionObserver = jest.fn().mockImplementation((_callback) => ({
      observe: jest.fn(),
      unobserve: jest.fn(),
      disconnect: jest.fn()
    }));
...
describe('Click handler - Debug mode', () => {
  ...
  it('should log debug information when debug mode is enabled', () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(
      expect.stringContaining('[AnyWP] Click Handler Registered:'),
      expect.any(String)
    );
...
describe('Click handler - waitForElement timeout', () => {
  ...
  it('should log error when element not found within maxWait', (done) => {
    ...
    setTimeout(() => {
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        '[AnyWP] Element not found: #never-exists'
      );
      consoleErrorSpy.mockRestore();
      done();
    }, 150);
